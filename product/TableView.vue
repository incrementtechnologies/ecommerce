      ...
      <tbody>
        <tr v-for="(item, index) in sorted" :key="index">
          ...
          <td>
            ...
            <label class="underline-on-hover" @click="redirect('/marketplace/product/' + (item.product_code ? item.product_code : item.code) + '/preview' )" v-if="item.type !== 'bundled'">{{item.title}} ({{item.volume}})</label>
            <label class="underline-on-hover" @click="redirect('/marketplace/product/' + (item.product_code ? item.product_code : item.code) + '/preview' )" v-else>{{item.title}}</label>
          </td>
